...
import os
from genetic_algorithm.src.rectangle import generate_N_ractangles
...
def generate_stack_of_strips(gene_list, rotation_list, rectangles, max_strip_width, it_rotates):
    ...
    for i in gene_list:
        ...
        if max_strip_width <= (space_used + rectangle_width):
            ...
            # because we are in a new strip the space used is equal to
            # the only rectangle's width in it. this being the value of rectangle_width
            space_used = rectangle_width
        else:
            # accumulates the widths of the rectangles of one strip
            # because it's lower than W and there is still space left.
...
def stats(population, rectangles, max_width, best_ind, best_fitness, average_fitness, best_fitness_ever):
    ...
    # stack_of_strips = generate_stack_of_strips(best_of_generation.gene_list,
    # best_of_generation.rotation, rectangles, max_width, it_rotates)
...
def stats_experimental(population, rectangles, max_width, best_ind, best_fitness, best_fitness_ever):
    ...
    best_fitness.append(best_of_generation.fitness)
    best_fitness_ever.append(min(best_fitness + best_fitness_ever))
    # stack_of_strips = generate_stack_of_strips(
    # best_of_generation.gene_list,best_of_generation.rotation, rectangles, max_width, it_rotates)

    return best_ind, best_of_generation, best_fitness, best_fitness_ever


def get_values_from_files(config):
    # os.chdir(".\\genetic_algorithm")
    # os.chdir(".\\instances")
    # dir = os.getcwd()
    ...
    # for value in values:
    #     dimensions = np.array(
    #         value.split())  # value= "2 10" that correspond to the width and the height of one rectangle.
    #     dimensions = dimensions.astype(
    #         int)  # so we split it and assign it to an array called
    #     # dimensions = [2,10] and convert each element to integer
    #     rectangles_values.append(tuple(dimensions))

    return len(config['boxes']), config['boxes'], config['width']
