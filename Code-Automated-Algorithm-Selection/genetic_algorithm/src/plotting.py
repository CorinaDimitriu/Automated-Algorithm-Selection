...
import os
from genetic_algorithm.src.utils import (max_height, generate_stack_of_strips,
                   get_average_fitness, get_values_from_files, sort_by_height)
from genetic_algorithm.src.rectangle import generate_N_ractangles
from genetic_algorithm.src.GLOBAL import RECTANGLES_NUMBER, instances
...
def generate_plots(fig, rectangles, stack, individual, max_strip_width, it_rotates, subtitle):
    ...
    for strip in stack:
        ...
        for i in strip:
            ...
            rectangle = matplotlib.patches.Rectangle((Xaxis, Yaxis), rectangle_width, rectangle_height,
                                                     edgecolor='black', facecolor=colors[indexColor], linewidth=0.3,
                                                     alpha=0.6)
            ...
            ax.annotate(str("R" + str(i)), (cx, cy), color='black', weight='normal', fontsize=10, ha='center',
                        va='center')
...
def plot_individual_info(individual, W, rectangles, RESULTS_FOLDER, it_rotates, file_name):
    initial_stack_of_strips = generate_stack_of_strips(individual.gene_list, individual.rotation, rectangles, W,
                                                       it_rotates)
