...
from genetic_algorithm.src import GLOBAL
from genetic_algorithm.src.rectangle import generate_N_ractangles, generate_rectangles
from genetic_algorithm.src.selection import select_tournament
from genetic_algorithm.src.crossover import crossover
from genetic_algorithm.src.mutate import mutate
from genetic_algorithm.src.population import create_starting_population
from genetic_algorithm.src.fitness import calculate_fitness
from genetic_algorithm.src.utils import generate_stack_of_strips, get_best_individual, get_average_fitness, \
    stats, get_values_from_files, calculate_stats_per_instance, save_to_file, print_stats, stats_experimental
from genetic_algorithm.src.plotting import plot_result, generate_plots, add_text_below, plot_individual_info, \
    print_individual, \
    print_best_individual, plot_stats, plot_all_instance_solutions
from genetic_algorithm.src.GLOBAL import POPULATION_SIZE, MAX_GENERATIONS, MUTATION_PROBABILITY, CROSS_OVER_PROBABILITY, \
    TOURNAMENT_SIZE, \
    RESULTS_FOLDER, RECTANGLES_NUMBER, MAX_WIDTH
from genetic_algorithm.src import GLOBAL
    ...
    for generation_number in range(MAX_GENERATIONS):
        ...
        for ind in crossed_offspring:
            if random.random() < MUTATION_PROBABILITY:
                mutated.append(
                    mutate(ind.gene_list, ind.rotation, W, set_of_rectangles, number_of_rectangles, calculate_fitness,
                           new_seed, it_rotates))
            else:
                mutated.append(ind)
...
def default_GA(number_of_rectangles, W, genes, it_rotates, seed, GENERATIONS, TOURNAMENT_SIZE):
    ...
    # Start inicial population
    population = create_starting_population(POPULATION_SIZE, W, set_of_rectangles,
                                            genes, calculate_fitness, it_rotates,
                                            seed)
    ...
    for generation_number in range(GENERATIONS):
        ...
        for ind1, ind2 in zip(selected[::2], selected[1::2]):
            if random.random() < CROSS_OVER_PROBABILITY:
                children = crossover(ind1, ind2, W, set_of_rectangles,
                                     calculate_fitness, seed + generation_number,
                                     it_rotates)
                crossed_offspring.append(children[0])
                crossed_offspring.append(children[1])
            else:
        ...
        for ind in crossed_offspring:
            if random.random() < MUTATION_PROBABILITY:
                mutated.append(
                    mutate(ind.gene_list, ind.rotation, W, set_of_rectangles,
                           number_of_rectangles, calculate_fitness,
                           seed + generation_number, it_rotates))
            else:
                mutated.append(ind)
        population = mutated.copy()
...
def experimental(number_of_runs, seed):
    ...
    number_of_rectangles, rectangles_values, max_width = get_values_from_files(instances)

    # print("------- Running instance %s ----------" % instances)
    # print("------- RECTANGLES ----------")
    ...
    for mode in modes:
        ...
        for run in range(number_of_runs):  # 20 runs
            best_individuals.append(
                GA(number_of_rectangles, rectangles_values, max_width, genes, mode, seeds[run], run))
        end = time.time()
    ...
    GAnr_median, GAnr_mean, GAnr_best_fitness, GAnr_worst_fitness, GAnr_stard_deviation, GAnr_solution, GAnr_stack = \
        calculate_stats_per_instance(best_individual_GAnr, number_of_rectangles, rectangles_values, max_width,
                                     False)
...
########################### ---- MAIN ---- ################################
def execute():
    options = {"rotation": 0, "experimental": 1, "values_mode": -1}
    ...
    if options["experimental"] == 1:
        ...
        # opt = multi_option_selector(
        #     ["1. Print results in console", "2. Save results in .txt files", "3. Generate plots", "4. All"])

        opt = 1
        # menu for printing or ploting
        if opt == 1:
            print("Done")
            return min(GAnr_statistics["Best fitness"], GAnr_statistics["Worst fitness"],
                       GAr_statistics["Best fitness"], GAr_statistics["Worst fitness"])
        elif opt == 2:
