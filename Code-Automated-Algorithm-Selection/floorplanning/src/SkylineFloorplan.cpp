...
void SkylineFloorplan::floorplan(vector<int>& w, vector<int>& h, vector<int>& indices, SkylineHeuristic sh) {
    ...
    string logfile = "D:/Facultate/Semestrul_2_Master/AEA/Code-Automated-Algorithm-Selection/floorplanning/log/skyline-" + to_string(numBlocks) + "b-" + sorting_type + "-" + enum_to_string(sh);
    if (guillotine) {
        logfile += "-guillotine-";
