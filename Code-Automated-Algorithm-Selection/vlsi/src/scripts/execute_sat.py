...
def vlsi_sat(w, n, dims, encoding_module, timeout=300, rotation=False):
    ...
    if not rotation:
        ...
        return results['best_coords'], results['best_l'], results['finish'], results['unsat'], results['actual_dimsX'], \
        results['actual_dimsY']
...
def main():
    ...
    if not arguments.no_create_output:
        ...
        output_file = os.path.join(output_folder_path,
                                   f'solution-{instance_file_name}')  # f'{output_folder_path}\\solution-{instance_file.name.split("/")[-1]}'
