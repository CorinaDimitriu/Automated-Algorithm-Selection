...
def main() -> None:
    ...
    parser.add_argument('model', metavar='model', type=str,
                        choices=MINIZINC_MODELS + [f'model_final{i}' for i in range(2)],
    ...
    # Open instance file
    instance_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                                 f'instances/{instance}.txt')
    ...
    try:
        ...
        # Send `UNKNOWN` error if MiniZinc returns
        # a `KeyboardInterrupt` signal. This is due to an internal MiniZinc bug.
